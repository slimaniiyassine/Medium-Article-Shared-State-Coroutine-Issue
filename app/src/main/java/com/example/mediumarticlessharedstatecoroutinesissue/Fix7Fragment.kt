...
class Fix7Fragment : Fragment() {

    private val viewModel: Fix7ViewModel by viewModels()
...
@Composable
fun Fix7Screen(viewModel: Fix7ViewModel) {
    Column(
        modifier = Modifier
            .padding(16.dp)
            .fillMaxSize(),
        ...
        Text(
            text = "Fixed Balance (Actor): $${viewModel.balance}",
            style = MaterialTheme.typography.bodyLarge
        )

        Button(
            onClick = { viewModel.startActorDeposit() },
            colors = ButtonDefaults.buttonColors(
                containerColor = Color(0xFF36454F)
            ),
            modifier = Modifier.padding(top = 16.dp)
        ) {
            Text("Start Actor Deposit ($1,000)")
        }
    }
}
