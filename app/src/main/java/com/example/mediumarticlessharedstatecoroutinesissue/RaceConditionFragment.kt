...
class RaceConditionFragment : Fragment() {
    ...
    private val viewModel: RaceConditionViewModel by viewModels()
...
fun RaceConditionScreen(viewModel: RaceConditionViewModel) {
    Column(
        ...
        Text(
            text = "Bank Balance: $${viewModel.balance}",
            style = MaterialTheme.typography.bodyLarge
        )

        Button(
            onClick = { viewModel.startUnsafeDeposit() },
            colors = ButtonDefaults.buttonColors(
                containerColor = Color(0xFF36454F)
            ),
            modifier = Modifier.padding(top = 16.dp)
        ) {
            Text("Start Unsafe Deposit ($1,000)")
        }
    }
}
